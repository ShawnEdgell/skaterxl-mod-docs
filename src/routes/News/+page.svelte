<script>
  // Add any page-specific JavaScript here
</script>

<div class="max-w-4xl mx-auto py-6">
  <h1 class="text-4xl font-bold mb-4">News</h1>

  <!-- Latest Update Section -->
  <div class="mb-8">
    <h2 class="text-2xl font-bold my-2">Latest Update - Skater XL on Nintendo Switch</h2>
    <p><i>November 27, 2023</i></p>
    <p>Skater XL is now available on Nintendo Switch, bringing the thrill of skateboarding to a wider audience with its portable gameplay.</p>

    <div class="relative" style="padding-top: 56.25%;">
      <iframe class="absolute top-0 left-0 w-full h-full" src="https://www.youtube.com/embed/r6_wjWiXpx8" frameborder="0" allowfullscreen title="Nintendo Switch Trailer"></iframe>
    </div>
  </div>

  <!-- Grabs Update Section -->
  <div class="border-t border-gray-300 pt-4 mb-8">
    <h2 class="text-2xl font-bold my-2">Grabs Update - A Glimpse into 2023's Promises</h2>
    <p><i>August 30, 2023</i></p>
    <p>The Grabs Update, unveiled earlier, showcased enhanced grab mechanics, a sneak peek into the exciting features planned for 2023.</p>
    
    <!-- Video for Grabs Update -->
    <div class="relative" style="padding-top: 56.25%;">
      <iframe class="absolute top-0 left-0 w-full h-full" src="https://www.youtube.com/embed/tRuprTKtJOo" frameborder="0" allowfullscreen title="Grabs Update Trailer"></iframe>
    </div>
  </div>

  <!-- 2023 Preview Section -->
  <div class="border-t border-gray-300 pt-4 mb-8">
    <h2 class="text-2xl font-bold my-2">2023 Preview - Anticipated Features Still Awaited in 2024</h2>
    <p>While 2023 was expected to be a big year for Skater XL with numerous updates, as we move into 2024, players are still eagerly awaiting the release of some of these anticipated features.</p>
    
    <!-- Video for 2023 Preview -->
    <div class="relative" style="padding-top: 56.25%;">
      <iframe class="absolute top-0 left-0 w-full h-full" src="https://www.youtube.com/embed/zIUTsIKBa_o" frameborder="0" allowfullscreen title="2023 Preview Video"></iframe>
    </div>
  </div>

  <!-- More news entries here -->

</div>

<style>
  /* Add any page-specific styles here */
</style>
